...
ABSOLUTE_PATH=$(realpath "$CURRENT_FILE_PATH")

echo "Running schema migration script from $ABSOLUTE_PATH"
...
function test_schema_compatibility() {
  ...
  echo "schema file path: $ABSOLUTE_PATH/schema/$SCHEMA_FILE_NAME.avsc"

  SCHEMA=$(cat "$ABSOLUTE_PATH/schema/$SCHEMA_FILE_NAME".avsc)
  ...
  RESPONSE_BODY=$(curl -s -X POST -H "Content-Type: application/vnd.schemaregistry.v1+json" \
    --data @"$TMP_SCHEMA_PAYLOAD_FILE" "$SCHEMA_REGISTRY_URL"/compatibility/subjects/"$AVRO_SUBJECT"/versions/latest)
...
function process_schema_file() {
  ...
  if [ "$DRY_RUN" == "true" ]; then
    test_schema_compatibility "$AVRO_NAMESPACE" "$AVRO_SUBJECT" "$SCHEMA_FILE_NAME"
  else
...
ATTRIBUTES_TOPIC="events.property.conformed.attributes.v1"

process_schema_file "com.property.conformed" "${ATTRIBUTES_TOPIC}-value" "attributes" "FORWARD_TRANSITIVE"
